...
jQuery.ketchup.validation("required","Dies ist ein Pflichtfeld.",function(g,b,c){g=b.attr("type").toLowerCase();return g=="checkbox"||g=="radio"?b.attr("checked")==true:c.length!=0}).validation("minlength","Dieses Feld muss mindestens {arg1} Zeichen lang sein.",function(g,b,c,d){return c.length>=+d}).validation("maxlength","Dieses Feld darf maximal {arg1} Zeichen lang sein.",function(g,b,c,d){return c.length<=+d}).validation("rangelength","This field must have a length between {arg1} and {arg2}.",function(g,
b,c,d,h){return c.length>=d&&c.length<=h}).validation("min","Must be at least {arg1}.",function(g,b,c,d){return this.isNumber(c)&&+c>=+d}).validation("max","Can not be greater than {arg1}.",function(g,b,c,d){return this.isNumber(c)&&+c<=+d}).validation("range","Must be between {arg1} and {arg2}.",function(g,b,c,d,h){return this.isNumber(c)&&+c>=+d&&+c<=+h}).validation("number","Muss eine Nummer sein.",function(g,b,c){return this.isNumber(c)}).validation("digits","Muss aus Zahlen bestehen.",function(g,b,c){return/^\d+$/.test(c)}).validation("email",
